...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($address_err) && empty($department_err) && empty($position_err) && empty($salary_err)){
        ...
        $sql = "UPDATE employees SET name=?, address=?, department=?, position=?, salary=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssi", $param_name, $param_address, $param_dept, $param_position, $param_salary, $param_id);
